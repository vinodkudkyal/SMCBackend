...
// Helper:  Random location within geofence
function getRandomLocation(geofence) {
  ...
  return {
    latitude: (point.latitude || point.lat) + (Math.random() - 0.5) * 0.001,
    longitude: (point. longitude || point.lng) + (Math.random() - 0.5) * 0.001,
  };
}
...
async function addDummyData() {
  try {
    ...
    for (const sweeperInfo of SWEEPERS) {
      ...
      let dayCounter = 1;
      for (const baseDate of ATTENDANCE_DATES) {
        const dateKey = yyyymmdd(baseDate);
        ...
        const alarmTimes = [
          new Date(baseDate.getTime() + 8 * 60 * 60 * 1000 + 30 * 60 * 1000), // 8:30 AM
          new Date(baseDate. getTime() + 10 * 60 * 60 * 1000 + 15 * 60 * 1000), // 10:15 AM
          new Date(baseDate.getTime() + 14 * 60 * 60 * 1000 + 45 * 60 * 1000), // 2:45 PM
        ];

        // Pattern:  Verified, Missed, Verified (to give variety)
        const eventTypes = ["verified", "missed", "verified"];

        for (let i = 0; i < 3; i++) {
          const alarmTimestamp = alarmTimes[i]. getTime();
          const eventId = new mongoose.Types.ObjectId().toString();

          let event = {
            id: eventId,
            alarmTimestampMs: alarmTimestamp,
            opened: false,
          ...
          if (type === "verified") {
            const location = getRandomLocation(sweeper. geofence);
            const openTime = alarmTimestamp + (Math.random() * 120000 + 30000);
            const verifyTime = openTime + (Math.random() * 180000 + 60000);

            event.opened = true;
            event.openedTimestampMs = openTime;
            event.responseMs = openTime - alarmTimestamp;
